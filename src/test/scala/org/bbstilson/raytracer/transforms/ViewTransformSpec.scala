...
class ViewTransformSpec extends UnitSpec {
  it should "handle default orientation" in {
    val t = ViewTransform.move(Point(0, 0, 0), Point(0, 0, -1), SceneVector(0, 1, 0))
    t shouldBe new MatrixDouble(MatrixUtils.identity)
  }
  ...
  it should "move the world" in {
    val t = ViewTransform.move(Point(0, 0, 8), Point(0, 0, 0), SceneVector(0, 1, 0))
    t shouldBe Translation(0, 0, -8)
  }

  it should "move to an arbitrary position" in {
    val t = ViewTransform.move(Point(1, 3, 2), Point(4, -2, 8), SceneVector(1, 1, 0))
